...
def solve(input_data):
    ...
    provinces_startswith_h = sorted([
        (input_data[i]['name'], input_data[i]['population'])
        for i in range(len(input_data)) if input_data[i]['name'][0] == 'H'
    ], key=lambda tmp1: tmp1[0])

    provinces_more_than_1m = sorted([
        (input_data[i]['name'], input_data[i]['population'])
        for i in range(len(input_data)) if input_data[i]['population'] > 10 ** 6
    ], key=lambda tmp2: tmp2[1], reverse=True)
